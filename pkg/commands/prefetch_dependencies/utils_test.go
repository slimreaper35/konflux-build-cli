...
func TestGetHostnameFromRemoteOriginURL(t *testing.T) {
	...
	t.Run("should get hostname from HTTPS origin URL", func(t *testing.T) {
		...
		g.Expect(executor.ExecuteInDir(tempDir, "git", "init")).To(Succeed())
		g.Expect(executor.ExecuteInDir(tempDir, "git", "remote", "add", "origin", "https://github.com/user/repo.git")).To(Succeed())
