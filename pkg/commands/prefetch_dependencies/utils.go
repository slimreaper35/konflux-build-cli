...
// Wrapper around the update-ca-trust command.
func updateTrustStore() error {
	...
	available, err := cliwrappers.CheckCliToolAvailable("update-ca-trust")
	if err != nil {
		return err
	}
	if !available {
		log.Warn("update-ca-trust CLI is not available")
		return nil
	}
