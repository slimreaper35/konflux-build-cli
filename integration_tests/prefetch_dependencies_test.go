...
func TestPrefetchDependencies(t *testing.T) {
	...
	t.Run("should prefetch dependencies with RPM input", func(t *testing.T) {
		...
		depsDir := filepath.Join(repoPath, "output", "deps", "rpm")
		info, err := os.Stat(depsDir)
		Expect(err).ToNot(HaveOccurred())
		Expect(info.IsDir()).To(BeTrue())

		// Check generated environment file.
		envFile := filepath.Join(repoPath, "cachi2.env")
		info, err = os.Stat(envFile)
		Expect(err).ToNot(HaveOccurred())
		Expect(info.Mode().IsRegular()).To(BeTrue())

		// Check generated SBOM file.
		sbomFile := filepath.Join(repoPath, "output", "bom.json")
		...
		filepath.WalkDir(filepath.Join(repoPath, "output"), func(path string, entry os.DirEntry, err error) error {
			if err != nil {
				return err
			}
